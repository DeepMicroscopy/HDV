...
from utils.dataset_adaptors import MidogDatasetAdaptor

from utils.constants import YOLO_LAYERS
...
def get_all_annotations(
    ...
    for file in files:
        ...
        label = subdata['label']
        domain = subdata[domain_col].unique().item()
        coords = subdata[['x', 'y']]
        if only_border:
...
def main(args):
    ...
    if 'yolov7' in str.lower(config_file.detector):
        layers = YOLO_LAYERS
    else:
